 ...
 *
 * APPROACH 1: MinHold
 * Buttons stay pressed for a minimum duration (150ms) even after touch release.
 * This ensures the menu polling loop has time to see the button state.
 */
#ifdef WEBOS
...
/* MinHold approach: track when each button was last pressed */
#define MENU_HOLD_MS 150  /* Minimum hold duration in milliseconds */
static Uint32 button_press_time[4] = {0};  /* UP, DOWN, MOK, MBACK */
static int button_pressed[4] = {0};        /* Currently pressed by finger */
...
/* Map zone index to button index (0=UP, 1=DOWN, 2=MBACK, 3=MOK) */
...
static void update_buttons(void)
{
    ...
    /* Reset finger-pressed state for menu buttons */
    for (i = 0; i < 4; i++) {
        button_pressed[i] = 0;
    }
    ...
    for (i = 0; i < MAX_FINGERS; i++) {
        if (finger_zones[i] >= 0 && finger_zones[i] < num_zones) {
            ...
            if (menu_mode) {
                /* Track which buttons are currently pressed by finger */
                int btn_idx = zone_to_button_idx(finger_zones[i]);
                if (btn_idx >= 0) {
                    button_pressed[btn_idx] = 1;
                    button_press_time[btn_idx] = now;  /* Update press time */
                }
            } else if (key >= 0) {
    ...
    /* MinHold: Build menu buttons from both currently pressed AND held buttons */
    current_menu_buttons = 0;
    if (menu_mode) {
        for (i = 0; i < 4; i++) {
            int is_active = button_pressed[i];

            /* Also active if within hold time */
            if (!is_active && button_press_time[i] > 0) {
                Uint32 elapsed = now - button_press_time[i];
            ...
            if (is_active) {
                switch (i) {
                case 0: current_menu_buttons |= PBTN_UP;    break;
                case 1: current_menu_buttons |= PBTN_DOWN;  break;
                case 2: current_menu_buttons |= PBTN_MBACK; break;
                case 3: current_menu_buttons |= PBTN_MOK;   break;
                }
            }
        }
    }
...
int webos_touch_init(void)
{
    ...
    printf("WebOS Touch [MinHold]: Initializing on-screen controls\n");
    printf("WebOS Touch [MinHold]: Buttons held for minimum %d ms\n", MENU_HOLD_MS);
    ...
    printf("WebOS Touch [MinHold]: %d touch zones defined\n", (int)NUM_TOUCH_ZONES);
    return 0;
}
...
void webos_touch_set_menu_mode(int in_menu)
{
    ...
    if (menu_mode != in_menu) {
        ...
        printf("WebOS Touch [MinHold]: Switched to %s mode\n", menu_mode ? "menu" : "game");
    }
}

int webos_touch_get_menu_buttons(void)
{
    /* MinHold approach: recalculate based on current time */
    update_buttons();
    return current_menu_buttons;
}
